...
# Purpose: Stop DB instance(s) of specific environment
...
stopDBInstance(){
 dbInstanceName=$1
 /usr/bin/python -c \
  "from rdsAdmin import RDSStop;;RDSStop().stop_db_instance('$dbInstanceName')" \
  > $LOGDIR/${dbInstanceName}.stop.json
}
